...
class TrajectoryUtils():
    ...
    def interpolateHeading(self, waypoints):
        ...
        while idx != (len(waypoints) - 1):
            ...
            for i in range(1, len(subtraj) - 1):
                ...
                ds = distEuclidean(subtraj_0, subtraj_1)/subtraj_len
                hdg_interp = wrapAngle(hdg_from + (d_hdg* ds))
                # replace heading
                hdg_from   = hdg_interp
                wp         = subtraj[i]
    ...
    def sampleTrajectoryThroughWaypoints(self, trajectory, with_stops, smooth_path, smoothing_la_dist, smoothing_sampling_step, velocity_limits, acceleration_limits):
        ...
        if with_stops:
            ...
            ts_sample = np.linspace(0, toppra_trajectory.duration, sampling_step)
            samples = toppra_trajectory.eval(ts_sample)            # Convert to Trajectory class
